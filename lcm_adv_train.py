...
from unet_as_discriminator import SAR2OptUNetv_as_Dis
from dataloader_pair import pair_Dataset_csv
...
discriminator = SAR2OptUNetv_as_Dis(
            sample_size=256,
            in_channels=4,
            out_channels=3,
            layers_per_block=2,
...
missing_keys, unexpected_keys = discriminator.load_state_dict(safe_load(args.unet_checkpoint), strict=False)
accelerator.print(f"Load Discriminator model from: {args.unet_checkpoint}")
accelerator.print(f"Discriminator model Loaded. ### missing key: {len(missing_keys)}, ### unexpected key: {len(unexpected_keys)}")
...
unet, discriminator, optimizer, lr_scheduler, optimizerD, lr_schedulerD = accelerator.prepare(
    unet, discriminator, optimizer, lr_scheduler, optimizerD, lr_schedulerD
)
...
for epoch in range(first_epoch, args.num_train_epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        with accelerator.accumulate(unet,discriminator):
            ...
            for p in disc_trainable_params:
                p.requires_grad_(False)
            with torch.autocast("cuda"):
                disc_logits = discriminator(
                    sample1=noisy_model_input_cat,
                    timestep1=start_timesteps,
                    sample2=torch.cat([model_pred, sar_image], 1),
                    timestep2=timesteps,
                )
                if args.gan_loss_type == "unsaturated":
                    unet_adv_loss = torch.mean(F.softplus(-disc_logits))
                elif args.gan_loss_type == "hinge":
                    unet_adv_loss = -torch.mean(disc_logits)
                elif args.gan_loss_type == "wgan":
                    unet_adv_loss = -torch.mean(disc_logits)
                else:
                    raise NotImplementedError
            loss = loss_lcm + unet_adv_loss
            # Backpropagate
            ...
            with torch.autocast("cuda"):
                disc_logits_student, disc_logits_teacher = discriminator(
                    sample1=noisy_model_input_cat.to(weight_dtype),
                    timestep1=start_timesteps,
                    sample2=torch.cat([model_pred.detach(), sar_image.detach()], 1),
                    timestep2=timesteps,
                    sample3=torch.cat([target.detach(), sar_image.detach()], 1).to(weight_dtype),
                    timestep3=timesteps
                )
            if args.gan_loss_type == "unsaturated":
                disc_loss_pred = torch.mean(F.softplus(disc_logits_student))
                disc_loss_true = torch.mean(F.softplus(-disc_logits_teacher))
            elif args.gan_loss_type == "hinge":
                disc_loss_pred = torch.mean(F.relu(1. + disc_logits_student))
                disc_loss_true = torch.mean(F.relu(1. - disc_logits_teacher))
            elif args.gan_loss_type == "wgan":
                disc_loss_pred = torch.mean(disc_logits_student)
                disc_loss_true = -torch.mean(disc_logits_teacher)
            else:
                raise NotImplementedError
    
            d_loss = disc_loss_pred + disc_loss_true
            # Backpropagate on the online discriminator model
        ...
        with torch.no_grad():
            if accelerator.sync_gradients and (global_step - 1) % args.validation_steps == 0:
                ...
                accelerator.wait_for_everyone()
        logs = {"loss_lcm": loss_lcm.detach().item(), "unet_adv_loss":unet_adv_loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0],"disc_loss_pred":disc_loss_pred.detach().item(),"disc_loss_true":disc_loss_true.detach().item()}
